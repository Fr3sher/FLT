    ...
    },
{
      ...
      markdown: [
        ...
        "",
        "**Sample prompts, LoRA rank and Memory mode** apply to both launch buttons.",
        "Rank defaults to 16; larger ranks use more memory. Memory mode defaults to",
        "Auto: low VRAM on for this PC, off for a rented GPU. Choose On or Off to",
        "override it; low VRAM trades speed for reduced memory use.",
        "Add up to four sample prompts, one per line (include your trigger word), to",
        "generate previews during training. They appear in the run graph after generation",
        "and collection. Sampling adds time and cloud cost; leave the field blank to",
        "disable it. Cloud retries and continuations keep the original run's settings.",
        "",
        ...
        "(no [vast.ai](" + vastGuideUrl('/') + ") key, no clips) stops there and says why; warnings are put to you once,",
        ...
        "read as one lineage rather than three unrelated runs. A pill shows its requested",
        "render when one exists, otherwise its *training sample*. Click its result chip",
        "to open the corresponding history. The save's actions keep **Play samples**",
        "available separately; `←`/`→` step through the training prompts. Clicking a run",
        "card opens its details. **Generate previews** compares selected H3 saves with",
        "one common take, using the Video Studio engine (see below). A run with no",
        "samples or requested previews simply shows plain pills.",
        "",
...

{
      ...
      markdown: [
        ...
        "motion**: choose **GPT — ChatGPT subscription**, or a model listed by your",
        "local Ollama or LM Studio server. GPT reuses the ChatGPT connection from",
        "Plugins → API image engines → Settings and its subscription model. It sends the staged",
        "frame and instructions to ChatGPT for Auto, Enrich and per-picture writing;",
        "it needs no local LLM and does not unload ComfyUI. If disconnected, reconnect",
        "in Plugins → API image engines → Settings. A failed request keeps your prompt unchanged;",
        "it never switches to a paid API key. The choice is independent: tuning the writer never re-points",
        ...
        "which model writes, and, for local models, how it samples. GPT uses its own",
        "sampling settings, so these local-only dials are hidden when it is selected. Two temperatures on purpose —",
    ...
    },
{
      ...
      markdown: [
        ...
        "those to fit your intent. JoyCaption reads for any writer, GPT included. It",
...
GUIDE.sections.push({
  chapter: 'using-the-app',
  anchor: 'share-a-video-checkpoint-on-civitai',
  markdown: [
    '## Share a video checkpoint on Civitai',
    '',
    'From a finished video training run, open its checkpoint and choose the Civitai publishing action. Connect the Publish to Civitai plugin and add your API key in its settings first. Review the model description, preview clips, license and visibility before publishing.',
    '',
    'The checkpoint stays in your run. Publishing creates a separate Civitai model or version; it does not replace your local file. If publishing is unavailable, check that the Civitai plugin is installed and enabled, the run has a finished checkpoint, and your API key is valid.',
  ].join('\n') + '\n',
})
