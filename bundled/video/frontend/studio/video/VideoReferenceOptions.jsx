...
export default function VideoReferenceOptions({ options, value, onChange, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <label className="flex flex-col gap-1 text-xs text-content-muted">Base
        <select value={value.base} onChange={(e) => onChange({ base: e.target.value })} className={select}>
      ...
      <label className="flex flex-col gap-1 text-xs text-content-muted">Reference acceleration
        <select value={value.accel || ''} onChange={(e) => onChange({ accel: e.target.value, steps: '' })} className={select}>
      ...
      {!remote && (base?.available === false || base?.ready === false || accel?.available === false) && (
