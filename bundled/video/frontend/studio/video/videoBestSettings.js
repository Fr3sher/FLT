...
export function bestToControls(best, options, currentOptions) {
  ...
  return {
    ...
    opts: { ...currentOptions, frames: s.frames, megapixels: s.megapixels,
...
export function bestUnavailable(best, options, mode) {
  ...
  const s = best.settings;
  if (![options.base_official, options.base_eros, options.base_light].includes(s.base_model)) return 'The saved base model is not available in this Studio.';
