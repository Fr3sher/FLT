...
def test_registration_owns_only_public_video_and_never_live(monkeypatch):
    ...
    installs, downloads, probes, blueprints, handlers, hooks, packs = {}, {}, {}, [], {}, {}, set()
    for name, attrs in {
    ...
    ctx = SimpleNamespace(dir=ROOT,
        register_node_pack=lambda key: packs.add(key),
    ...
    assert set(installs) | set(downloads) | packs == set(manifest['owns']['install_actions'])
