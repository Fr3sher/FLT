...
def _probe_file(path):
    ...
    from lds_video import video_probe
...
def _detect_source(path, fps_native=None, **options):
    ...
    from lds_video import shot_detect
...
def _caption_runtime() -> dict:
    ...
    from lds_video import video_caption
    from lds_sdk.video_host.capabilities import bank_scoring_gpu_available
...
def _embed_available():
    ...
    from lds_sdk.video_host.clip_image_encoder import unavailable_reason
...
def start_dedup(app, user_id, bank_id, threshold=None):
    ...
    already exist."""
    from lds_video.video_clip_search import load_embeddings
    _require_free_bank(user_id, bank_id)
...
def _dedup_job(bank_id, threshold):
    def run(job):
        from lds_video import video_clip_dedup
...
def _watermark_available():
    ...
    why not."""
    from lds_video.video_watermark import unavailable_reason
    return unavailable_reason()
...
def start_watermark(app, user_id, bank_id, rescan=False):
    ...
    from lds_sdk.video_host.capabilities import watermark_detect_gpu_available
    _require_free_bank(user_id, bank_id)
...
def _watermark_job(bank_id, rescan, use_gpu):
    def run(job):
        ...
        from lds_sdk.video_host.gpu import gpu_exclusive_vision_window
        from lds_video import video_watermark
...
def _safe_zone_job(bank_id, rescan):
    def run(job):
        from lds_video import video_safe_zone
...
def start_defects(app, user_id, bank_id, rescan=False):
    ...
    """
    from lds_video.video_defect_sweep import unavailable_reason
    _require_free_bank(user_id, bank_id)
...
def _defects_job(bank_id, rescan):
    def run(job):
        from lds_video import video_defect_sweep
...
def start_ai_check(app, user_id, bank_id, recheck=False):
    ...
    """
    from lds_video.video_ai_check import unavailable_reason
    _require_free_bank(user_id, bank_id)
    reason = unavailable_reason()
    if reason:
        raise RuntimeError(reason)
...
def _ai_check_job(bank_id, recheck):
    def run(job):
        from lds_video import video_ai_check
...
def start_camera(app, user_id, bank_id, rescan=False):
    ...
    """
    from lds_video.video_camera_motion import unavailable_reason
    _require_free_bank(user_id, bank_id)
    reason = unavailable_reason()
    if reason:
        raise RuntimeError(reason)
...
def _stream_video_to_disk(url, dest_path) -> str:
    ...
    from lds_sdk.video_host.scrape.netfetch import _validate_public_http_url
...
def _download_scrape_video(item, staging_dir) -> tuple:
    ...
    from lds_sdk.video_host.scrape.netfetch import MAX_DRIVER_BYTES
    from lds_sdk.video_host.scrape.netfetch import download_via_ytdlp
    from lds_sdk.video_host.scrape.netfetch import _validate_public_http_url
    ok_url, err = _validate_public_http_url(url)
    if not ok_url:
        logger.warning('video bank scrape: URL refused by the SSRF guard: %s', err)
