...
def video_bank_clip_thumbs_batch(bank_id):
    """Return indexed JPEGs for the grid in one response."""
    data = request.get_json(silent=True) or {}
    ids = [int(value) for value in (data.get('ids') or []) if str(value).isdigit()]
    if not ids:
    ...
    for position, clip_id in enumerate(ids):
        ...
        if path.is_file():
            ...
            output += struct.pack('>II', position, len(image))
            output += image
    return Response(bytes(output), mimetype='application/octet-stream',
