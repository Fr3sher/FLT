...
from lds_sdk.engine_errors import EngineError, EngineFatal, EngineRefused
...
# SubscriptionQuotaExceeded / SubscriptionUnavailable are defined in
# engine_errors (the core catches them) and re-exported from here so callers
# and tests keep naming them through this module.
from lds_sdk.engine_errors import SubscriptionQuotaExceeded, SubscriptionUnavailable  # noqa: F401,E402 — re-export, see above
...
def _raise_for_api_status(resp, *, model: str) -> None:
    ...
    if status == 403:
        # Preserve the provider's detail; a status alone cannot identify which
        # credential, project, or model access requirement caused the refusal.
        raise ChatGPTImageFatal(
            f'OpenAI refused the request for model "{model}" (HTTP 403){suffix} — '
            'check the provider\'s error, your key\'s project permissions and the '
            'model\'s access requirements. HTTP 403 alone does not confirm that '
            'the key is valid or that organization verification is the cause. '
            'Review the configuration in Plugins → API image engines → Settings')
    if status == 404:
        raise ChatGPTImageFatal(
