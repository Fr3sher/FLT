...
{"chapter": "settings-reference", "anchor": "api-image-engines-configuration-keys", "markdown": [
        ...
        "| `engines.chatgpt_image_model` | Image model the ChatGPT engine requests on the **API-key** lane. Free text; blank = the `CHATGPT_IMAGE_MODEL` environment variable if set, else `gpt-image-2`, the plugin default. Must accept reference images. The subscription lane ignores it. |",
      ...
      ].join("\n") + "\n"},
    {
      ...
      markdown: [
        ...
        "- **OpenAI API key** — powers the ChatGPT engine (`gpt-image-2`). **Test** checks the saved key or connected subscription; it does not verify provider access. This key is **optional if you connect a ChatGPT subscription** below — the subscription lane can run the ChatGPT engine on your plan's image quota instead.",
    ...
    },
    {
      ...
      markdown: [
        ...
        "- **ChatGPT (OpenAI) image model** → `engines.chatgpt_image_model`. Blank uses `CHATGPT_IMAGE_MODEL` if set, otherwise **`gpt-image-2`**, the plugin default.",
