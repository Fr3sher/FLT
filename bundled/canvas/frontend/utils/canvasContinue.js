/* ▶ Continue training FROM THE BOARD — the rules the LoRA Canvas needs to turn a
   ...
   Runs and Canvas both use the shared continuation dialog. */
...
/* The REQUEST a resolved dialog payload becomes: { url, body }.
   ...
   The supported routes are listed above. Two things it does
...
export function canvasContinueRequest(node, payload, { steps = [], masked = null } = {}) {
  ...
  const list = Array.isArray(steps) && steps.length ? steps : canvasContinueSteps(node);
  payload = explicitRunContinuation({ ...node, resume_steps: list }, payload);
  if (!payload) return null;
  const fromStep = payload.fromStep;
  const extra = { extra_steps: payload.extraSteps };
  if (fromStep != null) extra.from_step = fromStep;
  if (payload.expectedRecordId != null) extra.expected_record_id = payload.expectedRecordId;
  ...
  const body = { ...extra, ...selection };
  if (payload.lane && payload.lane !== 'local') {
    const lane = contributions('training.continue.lane', 'dataset').find((item) => item.id === payload.lane);
    return lane?.request?.({ node, body }) || null;
  }
  if (node.dataset_id == null) return null;
  return { url: `/api/dataset/${node.dataset_id}/train/continue`, body };
}
