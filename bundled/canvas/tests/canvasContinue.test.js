...
const canvas = fs.readFileSync(new URL('../frontend/components/canvas/LineageCanvas.jsx', import.meta.url), 'utf8');
...
test('cloud lane on a LOCAL run seeds a pod from the local file (continue-local)', { skip: !cloudLane }, () => {
  ...
  assert.equal(req.body.expected_record_id, 12);
...
test('local lane only resumes a local source through the dataset endpoint', () => {
  ...
  assert.equal(fromLocal.body.expected_record_id, 12);
});
...
for (const lane of ['local', 'cloud']) test(`overrides ride only when the dialog produced some (${lane})`, { skip: lane === 'cloud' && !cloudLane }, () => {
  ...
  const withOv = canvasContinueRequest(LOCAL,
    { lane, extraSteps: 1000, overrides: { lr_factor: 0.5 } }, { steps: [3500] });
...
test('the dialog waits for the lane inputs before it seeds itself', () => {
  ...
  // landed made a configured cloud lane read "needs a vast.ai API key",
...
test('disabling the cloud contribution keeps local continuation and closes cloud requests', () => {
  setEnabled([]);
