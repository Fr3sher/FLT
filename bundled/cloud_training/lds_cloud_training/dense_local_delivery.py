...
def disk_margin_bytes(cloud_cfg=None) -> int:
    from lds_cloud_training.hf_storage import GB
    if cloud_cfg is None:
        from lds_sdk.cloud_host import config as cfg
        cloud_cfg = cfg.get('cloud') or {}
    dense = (cloud_cfg or {}).get('full_transformer') or {}
...
def local_delivery_forecast(keeps=1, fp8_export=True, keep_bf16=True,
    ...
    from lds_sdk.cloud_host.services.fp8_export import estimate_fp8_bytes
...
def verify_local_file(path, expected_size=None) -> dict:
    ...
    26 GB of I/O.
    """
    from lds_sdk.cloud_host.services.fp8_export import Fp8ExportError, read_header
