...
def _app_context(app):
    global cq, vast_client
    from lds_cloud_training import cloud_quantize, vast_client as client
    cq, vast_client = cloud_quantize, client
    with app.app_context():
        yield app


@pytest.fixture(autouse=True)
def _tokens(monkeypatch, _app_context):
    from lds_cloud_training import hf_storage
    monkeypatch.setattr(hf_storage, "dense_storage_forecast", lambda *a, **kw: {"free_bytes": 10**15})
    monkeypatch.setenv("VAST_API_KEY", FAKE_TOKEN)
...
class _Vast:
    ...
    def destroy_instance(self, instance_id, **kwargs):
...
def vast(monkeypatch):
    fake = _Vast()
    for name in ('search_offers', 'create_instance', 'destroy_instance', 'list_instances'):
...
def test_a_successful_job_reports_the_file_and_destroys_the_machine(monkeypatch, vast):
    ...
    assert api.deleted == [f'_lds_fp8_result_{cq._receipt()["job_id"]}.json']
...
def test_an_uncertain_rental_keeps_proof_and_never_guesses_cleanup(monkeypatch, vast):
    ...
    assert cq._receipt()['pending'] and cq.has_unreleased_rental()
    assert len(vast.attempts) == 1
