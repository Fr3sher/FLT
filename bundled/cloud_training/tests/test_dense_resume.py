...
# Bound per test so module isolation cannot leave a monkeypatch on an old copy.
dense_pod_hub = None


@pytest.fixture(autouse=True)
def bind_dense_helper(isolated_product, monkeypatch):
    from lds_cloud_training import dense_pod_hub as helper
    monkeypatch.setitem(globals(), 'dense_pod_hub', helper)
...
@pytest.fixture()
def dataset_id(app):
    from app.services.face_dataset_service import create_dataset
    with app.app_context():
        return create_dataset('local', 'Dense resume', 'person').id
