...
test('Pexels key and attribution markup stay wired without nested controls', () => {
  ...
  const tileStart = panelSource.indexOf('<div key={it.url}')
  const buttonEnd = panelSource.indexOf('</button>', tileStart)
  assert.ok(tileStart >= 0 && buttonEnd > tileStart, 'selection button markup must remain present')
  const selectionButton = panelSource.slice(tileStart, buttonEnd)
  assert.match(selectionButton, /: toggle\(it\.url\)\}/)
  assert.doesNotMatch(selectionButton, /<a\b/i,
