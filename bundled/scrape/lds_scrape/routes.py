...
def _thumb_cache_dir():
    ...
    if _THUMB_CACHE_DIR is None:
        ...
        _THUMB_CACHE_DIR = Path(base) if base else Path(__file__).resolve().parents[3] / 'backend' / 'data'
...
def _fetch_image_to_temp(url):
    ...
    if not data:
        return None
    # The caller deletes returned paths. Always return a temporary file so
    # cached thumbnails remain available for later scans.
...
def scrape_face_filter():
    ...
    from lds_sdk.face_similarity import score_faces
