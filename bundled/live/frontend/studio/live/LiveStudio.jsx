...
const optionsUrl = () => '/api/video-studio/live/render-options';
import {
...
export default function LiveStudio() {
  ...
  const [options, setOptions] = useState(null);       // Live's shared H3 options
    ...
    <div className="flex flex-col gap-3" data-testid="live-studio">
      ...
      {options && options.ready === false && (
        <div className="rounded-xl border border-amber-400/40 bg-amber-400/10 p-3 text-sm">
          <p className="font-semibold text-amber-200">Local H3 rendering needs preparation</p>
          <p className="mt-1 text-content-muted">
            Prepare Live&apos;s H3 files and stream encoder from Plugins → Live → Settings,
            then start ComfyUI and return here. No other LDS plugin is required.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_22.5rem] lg:items-start">
        <div className="flex min-w-0 flex-col gap-3" data-probe-panel="live-take">
          ...
          <div className={running ? 'pointer-events-none opacity-60' : undefined} aria-disabled={running || undefined}>
            <H3LoraPicker apiBase="/api/video-studio/live" lockKey="live.lock.loraStrength" value={lora.lora} onChange={setLora}
