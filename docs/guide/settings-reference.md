...
| `bank_reranker.python` | empty | Advanced config: Qwen refinement interpreter. Setup records the managed Bank ML runtime; a selected external runtime is never modified or replaced by Setup. |
| `bank_reranker.models_root` | empty | Advanced config: pinned Hugging Face cache root for Qwen3-VL-Reranker-2B. Empty uses the app data folder's `models/bank_reranker`. Inference reads local files only. |
| `bank_reranker.device` | `cpu` | Advanced config: `cpu` or `cuda`. CUDA requires a compatible owner-provided BF16 runtime and the existing GPU reservation. The worker exits after every search; it has no idle-residency setting. |
