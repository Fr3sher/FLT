...
| `cloud.uplink_mbps` | Your upload speed in Mbit/s, used to forecast how long sending a checkpoint to a pod would take and what that costs in rented GPU time (default `0` = work it out). Leave it at `0`: the app **measures** your real speed on every transfer it makes and prefers what it measured over what you typed. Set it only to get a sensible forecast before your very first cloud run. |
