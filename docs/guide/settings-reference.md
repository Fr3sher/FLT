...
- **Extra reference images are ignored when generating variations.** Identity comes from the primary reference alone. The ✦ Edit reference dialog is the exception: there Krea also reads the **first** extra photo on the reference card — its edit graph has exactly one spare slot, so any further ones are unused.
  **What that slot is for:** the edit model was trained on two-input edits where the second image is a *different* subject — another person, or a scene to place yours in ("scene first, subject second"). Use it to compose ("put her in this room", "next to this person"), not to reinforce a face: another angle of the same person is off-label there and can duplicate them. Klein and the API engines use your extra refs in both places, and *they* are the ones that take extra angles to strengthen identity.
