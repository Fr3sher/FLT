...
A row marked **◐ in amber** is not broken: the tool is installed, it just isn't running (typically *launch ComfyUI to enable* for Klein, the Test Studio and the video rows whose verdict needs ComfyUI up). Those rows lead to the **ComfyUI API URL** field and its **Test** button rather than to an install you have already done. The counter at the top reads how many rows are ready plus, when it applies, how many are waiting on a process.
...
#### Krea hi-res fix (second pass)

A second sampling pass at a higher latent resolution on the Krea **generation** graph — the Test Studio grid and the LoRA Canvas (not the Krea 2 Edit engine above, which is a different graph). Krea composes at the resolution it samples: asked for the final size in one pass it loses framing and starts duplicating subjects; kept small it caps the detail no upscaler can invent afterwards. Sampling small, upscaling the latent and re-sampling lets the model draw that detail instead. Core ComfyUI nodes only — nothing to install. These are the **defaults**; the Studio's Sampling section can override the factor and the rewrite per run (its "Settings default" option names what is set here), and a cell keeps its own values so a 🔄 resume renders what it replaces.

- **Second pass** → `krea_hires.scale`. `1.0` = **off** (default) — and off adds no node at all, so the graph is exactly what it was before this setting existed. `1.25`–`2.0` = the latent upscale factor between the two passes; `1.5` is the value of the reference workflow this was ported from. ×2 on the latent is ×4 the pixels, and the second pass pays that in time and VRAM.
- **How much it may rewrite** → `krea_hires.denoise`. Range `0.05`–`1`, default **`0.5`**. The dial of the feature: near `1` the second pass ignores the first and renders a different picture at the larger size; too low and it costs a full pass to change nothing. `0.5` keeps the composition and rewrites the texture.
- **Second-pass steps** → `krea_hires.steps`. Default **`0`** = inherit the first pass's count, which is the sane default (at a denoise below 1 the pass only walks part of the schedule anyway). Read whenever the pass runs — including a run that armed it from the Studio while this switch is off.
...
- **The dataset's extra reference images are never used by Krea** — not when generating variations, and not when editing. Identity comes from the primary reference alone. Klein and the API engines are the ones that read those extra angles to strengthen identity.
...
**Shortcut from the workspace.** The multi-reference instructions are also reachable from **Add images ▸ Extra refs ▸ ✎**, without opening Settings. That modal shows **both** of them — `face_multi` (Nano Banana / ChatGPT) and `klein_identity` (Klein **and** Krea 2 Edit) — because the two engine families do not share one text, and badges **every** prompt your ticked engines actually use (a Klein + ChatGPT run really uses both). It edits the prompts of the **open dataset's subject type**, and says which one in its title and intro; edits made there are the same settings as here, for that subject.
...
- **Generation time limit (minutes)** → `comfyui.generation_timeout_minutes`. Default **15**, range **0–1,440**. Increase this for slow hardware, or use **0** to wait without an elapsed-time limit. It applies when a new ComfyUI job starts; cancellation and checks for a disconnected worker remain active. If a finite limit expires while ComfyUI is still working, LDS holds the queue until the uncertain job is reconciled rather than submitting overlapping work.
...
- **Advanced: ComfyUI folder overrides** → `comfyui.output_dir`, `comfyui.input_dir`, `comfyui.models_dir`, `comfyui.loras_dir`. All default **empty**, and empty is what you want unless ComfyUI runs on folders of its own — a ComfyUI started with `--output-directory`, `--input-directory` or `--models-directory` does *not* keep its files under the install directory, so without an override here the app reads and writes in the wrong place. Each field **shows the folder it falls back to while empty**, computed with the very function the app uses at runtime, so the effective path is never something you have to work out; a path that isn't on disk is flagged in amber rather than failing silently mid-generation. If ComfyUI is running, the app asks it which folders it was launched with (it reports its own command line via `/system_stats`) and offers them in one click — nothing is ever guessed from a folder layout, so no suggestion appears when ComfyUI is unreachable, predates that field, or was started with no custom folders.
  ...
  For `input/` there is a third check, and it catches what neither of the other two can: whether the folder you configured is **the one ComfyUI actually reads**. Present and writable is not the same as ComfyUI's — a second install, a portable copy, or a `--input-directory`/`--base-directory` on ComfyUI's command line all leave the app writing happily into a folder nobody reads. Nothing on disk shows it, so if ComfyUI is running the app simply **asks it**: it writes a marker into the folder and requests that file back over ComfyUI's own `/view`. A "no" is amber here, with the folder you configured *and* what ComfyUI reported about its own (quoted from the command line it echoes in `/system_stats`) — instead of the generation that used to die on ComfyUI's *"Invalid image file"* with nothing in the app to read (GitHub #64).
...
**And the two do different things to your file.** LaMa repaints the marked zones and leaves every other pixel exactly as it was. **Klein works in two steps**: the detected zones are erased on the photo (LaMa, or OpenCV when LaMa is not installed), and then the **whole photo** is re-rendered with the instruction *remove watermark*. The erasing is what stops the model handing your mark back; the whole-photo pass is what also clears a mark tiled across the frame, or one the detector boxed in the wrong place. It regenerates the rest of the image at the same time — the frame travels at 2 MP at most by default, and comes back at your file's own dimensions unless you say otherwise (both are dials now, see below). There is no zone-only Klein clean any more; **↩ Restore original** is the way back from a render you don't like. The **✦ Repair** action is unaffected — a repair you aim at a drawn box still leaves everything outside it byte-identical.

**The Klein clean has three dials of its own**, and they are not on this screen: they sit next to the **Klein model** choice in the Bank's *Level 3* panel and the dataset's **Clean** bar, because that is where you judge the result. Both surfaces edit the same stored values, so setting one arms the other — and the review lightbox's per-image Clean uses them too.

- **Prompt sent to Klein** → `watermark_clean.klein_prompt`. Default **`remove watermark`**. The instruction Klein is actually given when it re-renders the photo — visible now, and editable, so a mark that survives leaves you something to turn. Enter saves; **Reset to default** puts the three shipped words back, and an empty box means the default rather than an empty instruction. Short beats long here: the long reconstruction sentence belongs to **✦ Repair**, and asked to clean a whole photo Klein answers the short instruction measurably better. This is *not* the Repair prompt and never becomes one — a prompt typed here can never turn your clean into a zone-only repair.
- **Processing size** → `watermark_clean.klein_max_mp`. Default **`2.0`** MP, offered as 1 / 1.5 / 2 / 3 / 4 and clamped to **0.5–4.0**. How large the photo is scaled to before Klein re-renders it. Higher means finer regenerated detail, and more VRAM and more time — roughly with the pixel count, so 4 MP is about twice 2 MP's work per step, on a card that is probably also running ComfyUI. 4 MP fits a 24 GB card; on a smaller one, or with a training run alongside, stay at 2. A photo already smaller than the setting is sent as it is: **nothing is ever enlarged**, so raising this changes nothing for small images. This dial belongs to the 🧽 clean only — the ✦ Repair lane keeps its own fixed 2 MP, because it composites its render back and gains nothing outside the mask.
- **Write back** → `watermark_clean.klein_output`. Default **`original`**. `original` resamples the render back to your file's own dimensions, which is what has always shipped and what keeps a clean from changing the shape of a dataset image. `render` writes the render as it came back, with no second resample — it keeps that detail, and **the file changes dimensions**: it comes out *smaller* than your original whenever the photo was above the processing size above. On a photo already under that size the two modes produce the same file. **↩ Restore original** is the way back either way.

**What actually ran is in the log.** Because the prompt is editable, the source no longer answers "which words cleaned this batch?" — so every clean writes one line to 🪵 Server log (Settings ▸ Maintenance) naming the prompt it sent, the size the frame travelled at and the write-back mode, *before* the render, so a run that fails or times out still says what it was asked to do.

**Klein removal is not guaranteed**, because it is a generative pass rather than a mask. On the shipped settings a wall-to-wall tiled mark scored 0.69 before and 0.16 after, with its twelve detected zones down to none, and a photo carrying seven distinct logos came back with all seven gone. What can still survive is a mark **nothing detected** — no zone means nothing erased it, and the model is free to keep it. Lowering the denoise does not help — at 0.7 and 0.4 nothing at all is removed, which is why that value is fixed at 1.0 and is not offered as a dial.

**Why the erasing step exists, measured.** Without it — the whole photo alone, no zones erased — the model does not always delete a mark it can still see: it *redraws* it. On that test photo a round logo came back as a plausible **moon in the sky**, twice in one pass, in three runs out of three. The watermark detector then counts **zero** zones on the result, so the app reports the image cleaned and nothing downstream disagrees. Erasing the zones first removed every trace of it. **Klein cleaning therefore needs the ML extras** (the same install LaMa uses) — without them the pass reports itself unavailable and skips the row rather than rendering blind.
...
**Cloud ([vast.ai](https://cloud.vast.ai/?ref_id=683073)) internals** — knobs for after the real-world smoke test; the UI-exposed cloud settings above are the ones you'll normally want:
...
| `watermark_clean.klein_prompt` | The instruction the 🧽 Klein clean sends when it re-renders the photo (default `remove watermark`). Empty = that default. Edited next to the Klein model choice on both the Bank panel and the dataset Clean bar; never routed into the ✦ Repair lane. |
| `watermark_clean.klein_max_mp` | Megapixels the photo is scaled to before Klein re-renders it (default `2.0`, clamped 0.5–4.0). Higher = finer regenerated detail, more VRAM and more time. Never enlarges a smaller photo. The ✦ Repair lane keeps its own fixed cap. |
| `watermark_clean.klein_output` | What size the cleaned file is written at: `original` (default — resampled back to your file's own dimensions) or `render` (written at the render's size, so **the file changes dimensions**). |
...
| `klein.generation_lora_presets` | Named generation-LoRA stacks (default empty) picked per run in Klein tuning; each has a name and up to 8 `{file, strength}` rows. Built in Settings → Image engines; the **strengths** of the picked preset are also editable from the ✨ Upscale & improve window (same values, app-wide). |
