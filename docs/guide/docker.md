...
The in-app source updater cannot replace the immutable `/app` files in an image. For a GitHub ZIP installation, double-click **`update-docker.bat`**: it downloads the latest stable FLT release, keeps the previously selected launcher, rebuilds the image, and preserves `.env`, app data, ComfyUI folders, bank sources, `ollama-data/` and the generated external-ComfyUI override. Pass `main` to follow FLT's maintained branch; `v2` remains an alias for older commands.
...
The image installs `backend/requirements.txt` only. Optional scraping and ML extras can be installed from LDS; local ai-toolkit training remains a separate host/cloud tool.
