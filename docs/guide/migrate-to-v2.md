# Upstream LDS V1 to V2 branch migration

**This branch-switching tool is for installations cloned from `perfectgf/lora-dataset-studio` only. Do not run it on a `Fr3sher/FLT` installation. FLT keeps its maintained v2 code on `main`; use the [FLT installation and update guide](installation.md) instead.**

In the upstream repository, the maintained Git branch is now `v2`. The former `main` is named `v1` and is
