...
test('every bank image surface asks for the version key, edit generation included', () => {
  ...
  // Review keeps the fork's smaller, batchable WebP surface, but bypasses any
  // prefetched blob and versions that URL as soon as a turn/edit changes pixels.
  assert.match(lightbox, /\/review-file\/\$\{id\}\$\{imageVersionQuery\(img\)\}/)
