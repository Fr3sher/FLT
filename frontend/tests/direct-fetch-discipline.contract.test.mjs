...
// helpers that attach the token (`postForm(` / `postFormWithCsrfRetry(`) or set
// `X-CSRFToken` by hand
...
const carriesToken = (text) => {
  ...
  return /\bpostForm(?:WithCsrfRetry)?\s*\(/.test(code)
    || /['"]X-CSRFToken['"]\s*:/.test(code)
}
...
test('a comment that mentions the token does not count as sending it', () => {
  ...
  assert.equal(carriesToken('const r = await postFormWithCsrfRetry(url, form)'), true)
})
...
test('the FormData rule catches every shape that slipped past its first version', () => {
  ...
  assert.equal(carriesToken('const fd = new FormData(); postFormWithCsrfRetry(url, fd)'), true)
