...
for (const product of products) {
  test(`${product.id} alone exposes each declared preparation action`, () => {
    ...
    if (product.id === 'video') {
      // The performance panel uses action ids returned by the server's
      // availability probe, so they do not appear as literal JSX props.
      const panel = fs.readFileSync(path.join(BUNDLED, 'video', 'frontend', 'studio', 'video', 'VideoPerformanceOptions.jsx'), 'utf8')
      const probe = fs.readFileSync(path.join(BUNDLED, 'video', 'lds_video', 'h3_performance.py'), 'utf8')
      for (const [key, action] of [['spectrum', 'h3_spectrum_nodes'], ['fast', 'h3_fast_writer']]) {
        assert.ok(panel.includes(`action={status.${key}.action}`), `${key} preparation is missing from the performance panel`)
        assert.ok(probe.includes(`'${key}':`) && probe.includes(`'action': '${action}'`), `${key} probe does not name ${action}`)
