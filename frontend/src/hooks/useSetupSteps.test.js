...
test('installCatalog lists every app-installable component, present + available', () => {
  ...
  assert.deepEqual(
    ...
    ['face_scoring', 'masks', 'watermark_inpaint', 'video_text', 'ollama_model',
      ...
      'h3_clipproj_nodes', 'h3_video_vae', 'h3_video_vae_int8', 'h3_audio_vae',
      'h3_turbo_lora', 'h3_parasyte_lora', 'h3_dareties_lora', 'h3_base_light',
      'h3_vdn_stage', 'h3_vdn_stage_int8', 'h3_ref_base', 'h3_ref_base_light',
      'h3_ref_base_eros', 'h3_ref_turbo_4_lora', 'h3_ref_turbo_8_lora',
      'h3_reference_nodes', 'h3_attention_nodes',
  ...
  for (const c of Object.values(cat)) {
    if (!c.action.startsWith('h3_') || ['h3_base', 'h3_text_encoder', 'h3_video_vae',
      'h3_audio_vae', 'h3_turbo_lora', 'h3_parasyte_lora', 'h3_dareties_lora'].includes(c.action)) {
...
// ── ChatGPT: two lanes, one engine ───────────────────────────────────────────
// Setup used to expose only the paid API key, so a Plus/Pro subscriber met
// "OpenAI API key — ○ Not set" and concluded the engine costs money. These lock
