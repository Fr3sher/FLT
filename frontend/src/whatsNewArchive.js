// =====================================================================
//  🎁 What's new — THE ARCHIVE (entries shipped before 2026-08-11)
// =====================================================================
//
//  Same feed as frontend/src/whatsNew.js, older tail. Split out because 500+
//  entries were compiled into the ENTRY bundle to show a panel that opens on
//  the newest ten — and because whatsNew.js was the merge-conflict point of
//  every single wave (three branches out of three, twice over). This file is
//  loaded as its own lazy chunk when the panel's "Show older updates" is
//  pressed, and nothing else imports it.
//
//  RULES (they differ from the live file's):
//  • An entry moves HERE only after it has shipped in a tagged release —
//    release notes are built from the LIVE file's id diff, so archiving an
//    unshipped entry would silently cost it its release notes.
//  • `id` and `date` stay exactly as they were (the "seen" marker keys on
//    ids). `to:` is REMOVED on the way in: a months-old "Try it" would point
//    at routes and section ids that may have moved, and dropping it keeps
//    this file import-free — no live registries to validate against.
//  • Order inside is the live file's source order at split time (roughly
//    newest-first; display always sorts by date desc, id desc anyway).
//  • Entries arrive here from the live file's tail and are never edited.
// =====================================================================

// Newest first, all older than the live file's oldest.
export const WHATS_NEW_ARCHIVE = [
