 ...
 *  - a pin that did NOT resolve   → names what is loaded INSTEAD (warning), which
 *    is the case a typo used to hide completely. */
export function kreaBaseNote(pinned, resolved) {
  ...
  if (tail(got).toLowerCase() !== tail(pin).toLowerCase()) {
    return { tone: 'warn',
             text: `“${pin}” was not found under any krea folder — runs load ${got} instead.` };
  }
