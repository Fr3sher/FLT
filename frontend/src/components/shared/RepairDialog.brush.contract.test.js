...
test('Escape peels one layer, not two', () => {
  ...
  assert.match(generated, /if \(repairOpen \|\| improveOpen\) return;\s*if \(pluginLayersRef\.current\.any\(\)\) return;/);
  assert.match(generated,
    /if \(repairOpen \|\| improveOpen\) return;\s*if \(pluginLayersRef\.current\.any\(\)\) return;[^]{0,700}if \(e\.key === 'ArrowLeft' && onPrev\)/);
