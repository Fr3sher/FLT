    ...
    <li className={`relative overflow-hidden rounded-lg border border-border bg-surface ${STATUS_RING[img.status] || ''}`}>
      <button type="button" onClick={onToggle}
        ...
        {/* ?r=/?e= are cache busters, not parameters the server reads: the thumb
            ...
            moves the URL too — see bankEdits.imageVersionQuery. Untouched rows
            still use the batched blob to avoid one round trip per tile. */}
        <img src={imageVersionQuery(img)
          ? `/api/bank/${bankId}/thumb/${img.id}${imageVersionQuery(img)}`
          : (thumbSrc || `/api/bank/${bankId}/thumb/${img.id}`)}
