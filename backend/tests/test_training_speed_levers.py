...
def test_fork_krea_lora_extended_batch_size_survives_the_merge(app, tmp_path):
    with app.app_context():
        ds, folder = _prepare(tmp_path, batch_size=12)
        p = _cfg(ds, folder)
        snap = lt.launch_settings_snapshot(ds)
    assert p['train']['batch_size'] == 12
    assert snap['batch_size'] == 12
...
def test_the_panel_is_told_the_effective_value_and_the_choices(app, tmp_path):
    ...
    assert eff['batch_size_choices'] == list(range(1, 13))
...
def test_other_families_keep_the_narrower_batch_choices(app, tmp_path):
    with app.app_context():
        ds, _ = _prepare(tmp_path, family='zimage', batch_size=4)
        eff = lt.effective_train_settings(ds)
        with pytest.raises(ValueError, match='batch_size'):
            lt.update_train_settings(LOCAL_USER, ds.id, {'batch_size': 5})
