...
def test_history_cursor_comes_from_the_page_not_an_old_carried_source(client, app):
    """A render source can be far older than the page that carries it.

    It must not become the next cursor or the clips between it and the real page
    boundary disappear from the client's history.
    """
    from app.extensions import db
    from app.models import VideoTestClip
    with app.app_context():
        ids = [_clip(app, job_id=f'job-page-{i}') for i in range(5)]
        newest = db.session.get(VideoTestClip, ids[-1])
        newest.nr_of = ids[0]
        db.session.commit()

    first = client.get('/api/video-studio/clips?limit=2').get_json()
    assert [row['id'] for row in first['clips']] == [ids[4], ids[3], ids[0]]
    assert first['next_before'] == ids[3]
    assert first['has_more'] is True

    second = client.get(
        f'/api/video-studio/clips?limit=2&before={first["next_before"]}'
    ).get_json()
    assert [row['id'] for row in second['clips']] == [ids[2], ids[1]]
    assert second['next_before'] == ids[1]
    assert second['has_more'] is True

    exact = client.get('/api/video-studio/clips?limit=5').get_json()
