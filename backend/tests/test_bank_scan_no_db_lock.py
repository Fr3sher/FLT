...
def test_the_duplicate_regrouping_lets_other_writers_through(file_db):
    ...
    # and a green property reported as a regression. A later hosted runner
    # recorded a 0.26 share with 812 successful writes and no lock refusal:
    # thread scheduling noise can cross the old 0.25 line. The shape this guards
    # against is not slow, it is PARKED: the old code held the lock for 626 ms
    # of a 0.9 s phase, a share of 0.70. A 0.35 line separates those cases while
    # leaving room for runner scheduling. Sensitivity does not rest on this
    # number anyway —
