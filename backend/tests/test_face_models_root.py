...
def test_every_face_surface_hands_its_child_the_resolved_root(
    ...
    def fake_scorer(python, payload, *a, **k):
        ...
        key = 'generalized-scorer' if 'refs' in request else 'dataset-scorer'
        seen[key] = request['models_root']
    ...
    with app.app_context():
        ...
        fsim.score_faces([str(ref)], [str(img)], lenient=True)
        face_mask.detect_faces([str(img)])

    assert set(seen) == {'faces', 'folder-check', 'folder-scan',
                         'dataset-scorer', 'generalized-scorer', 'mask-detector'}
