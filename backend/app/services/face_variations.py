...
def wrap_variation_klein(prompt: str, nsfw: bool = False, framing: str | None = None,
    ...
    return _compose_edit_prompt(prompt, nsfw=nsfw, framing=framing, suffix=suffix,
                                subject_type=subject_type, label='',
                                concrete_outfit=False, markings_lock=False)
...
def _compose_edit_prompt(prompt: str, *, nsfw: bool, framing, suffix: str,
    ...
    copies to disagree. The two flags are the only real difference between the
    engines today (Krea names a concrete garment and holds the skin; Klein does
    not), so they stay parameters rather than duplicated bodies.
    ...
    body = _append_suffix(prompt, suffix)
    if concrete_outfit:
        body = krea_outfit_directive(body, label)
    body = apply_directive_overrides(body)
