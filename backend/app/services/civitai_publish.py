...
_UA = f'flt-fresh-lora-trainer/{APP_VERSION} (Civitai publisher)'
...
_PART_RETRIES = 3
_sleep = time.sleep             # narrow test seam; do not monkeypatch global time.sleep

GITHUB_URL = 'https://github.com/Fr3sher/FLT'
...
def build_description(ds, rec, step, inspection=None) -> str:
    ...
    parts = [f'<p>{kind} LoRA for <strong>{fam}</strong>, trained with '
             f'<a href="{GITHUB_URL}">FLT - Fresh LoRa Trainer</a>.</p>']
    if trigger:
