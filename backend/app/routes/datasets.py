...
def dataset_image_rotate(image_id):
    ...
    option of /crop: the crop lane RESAMPLES (it rescales the box to a 1024 px
    long side), which costs detail whatever encoder runs afterwards, while a
    quarter turn must not cost anything a turn does not require — this one keeps
    the file's real format and its pixels."""
    data = request.get_json(silent=True) or {}
    try:
