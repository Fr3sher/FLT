...
- **🎬 Video LoRAs** — *the dataset half exists, training launches from the app and the result is tested in-app* (see **Video Bank** and **🎬 Video Test Studio** above): shot detection, quality measures (motion, exposure, freeze, audio), captions that describe the action, keyword search across shots, target-aware cutting into a trainable folder, a ▶ Train button that runs the set through your local ai-toolkit or a rented pod, and a 🎬 Video Test Studio that renders the resulting LoRA, one clip per start frame on a fixed seed. What remains is proving the targets beyond Wan 2.2 with a finished run each. Community-driven.
