...
| **Full-model training (Krea 2)** | Train the whole transformer instead of an adapter. The finished master lands on your own disk and is verified before the pod is destroyed, appears in 📦 Checkpoints & LoRAs next to the ~10 GB fp8 twin ComfyUI loads, and can be continued from the step written inside it. The lane is narrow on purpose: cloud only, Krea 2 Raw only, no custom base — Turbo is refused as a lane nobody has measured, not an impossible one — and continuing needs the Hugging Face backup copy, so a run delivered to this computer only cannot be resumed |
